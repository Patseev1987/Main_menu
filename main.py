...
def delete_contact():
  ...
  delete_object=""
  while flag_delete:
    delete_object=input("Enter the contact number which you want to delete: ") 
...
def change_contact():
  ...
  delete_object=""
  while flag_delete:
    delete_object=input("Enter the contact number which you want to change: ") 
    if delete_object.isdigit() and 0<int(delete_object)<last_id:
      flag_delete=False      
    else:
      print("You entered the wrong number. Try again.")
  ...
  if get_check(changed_string,all_data):    
    with open(file_base, "w", encoding="utf-8") as f:
      for i in all_data:
        if int(delete_object) != int(i.split()[0]):
          new_string=""
  ...
  while search_flag:
    ...
    match answer:
      ...
      case "2":
        ...
        name_flag=True
        while name_flag:
          search_name=input("Enter a surname for search ")
          if search_name.strip()=="":
            print("You didn't enter any information. Try again.")
          else:
            name_flag=False
...
"3. Back\n")
    match answer:
      case "1":
        export_flag=False
        import_base_name=input("Enter a imported base without (.txt)\n")
      ...
      case "2":
        export_flag=False
        export_name_base=input("Enter a exported name base without (.txt)\n")
